function [state, Q, d] = initialStateEllipsis(Px,Py, Dx1, Dy1, Ex1, Ey1, Dx2, Dy2, Ex2, Ey2, Cx1, Cy1, Cx2 , Cy2, R1, R2, delta) % intersectY)
    ...
    [d_retaD1E1, lambdaD1E1, ~ , ~, Q01] = dist_reta(Dx1, Dy1, Ex1, Ey1, Px, Py);
    [d_retaE2D2, lambdaE2D2, ~ , ~, Q34] = dist_reta(Ex2, Ey2, Dx2, Dy2, Px, Py)
    ...
    elseif( (lambdaD1E1 >= 0) && (lambdaD1E1 <= D1E2) && (lambdaE2D2 >= 0) && (lambdaE2D2 <= E1D2) )
        % entra no segmento de reta mais perto
        if((d_retaD1E1 < d_retaE2D2))
    ...
    elseif ( (lambdaD1E1 >= 0) && (lambdaD1E1 <= D1E2) )

        if(min([d_retaD1E1 d_arcoE1E2 d_arcoD2D1]) == d_retaD1E1)
            if (Px >= Dx1 && Px < intersectX)
                state = 2;
                Q = Q01;
                d = d_retaD1E1;
        ...
        elseif(min([d_retaD1E1 d_arcoE1E2 d_arcoD2D1]) == d_arcoE1E2)
            state = 1;
            Q = Q2;
            d = d_arcoE1E2;
        
        elseif(min([d_retaD1E1 d_arcoE1E2 d_arcoD2D1]) == d_arcoD2D1)
            state = 3;
            Q = Q5;
            d = d_arcoD2D1;
    ...
    elseif ((lambdaE2D2 >= 0) && (lambdaE2D2 <= E1D2)) %ponto contido apenas no segmento E1D2 
        
        if(min([d_retaE2D2 d_arcoE1E2 d_arcoD2D1]) == d_retaE2D2)
            if (Px < Ex1 && Px > intersectX)
                state = 2;
                Q = Q34;
                d = d_retaE2D2;
        ...
        elseif(min([d_retaE2D2 d_arcoE1E2 d_arcoD2D1]) == d_arcoE1E2)
            state = 1;
            Q = Q2;
            d = d_arcoE1E2;
        
        elseif(min([d_retaE2D2 d_arcoE1E2 d_arcoD2D1]) == d_arcoD2D1)
            state = 3;
            Q = Q5;
            d = d_arcoD2D1;
