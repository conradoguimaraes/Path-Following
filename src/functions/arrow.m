...
% Copyright (c)1995-2021, Dr. Erik A. Johnson <JohnsonE@usc.edu>, 5/02/2023
% https://viterbi.usc.edu/directory/faculty/Johnson/Erik

% Revision history:
%    5/02/23  EAJ  Enable using the 'Parent' property
%    6/26/21  EAJ  viewmtx() is slightly inaccurate (round-off) when AZ & EL
%                    are multiples of 90 degrees; fix this
...
for k=firstprop:2:nargin,
	...
	elseif strncmp(prop,'parent' ,6),   extraprops(1,end+1:end+2) = {varargin{k} val}; % 5/2/2023
	else,
...
while (any(axnotdone)),
	...
	try % works in older MATLAB
		curT = get(curax,'Xform');
	catch
		az_el = get(curax,'View');
		curT = viewmtx(az_el(1),az_el(2));
		if all(rem(az_el,90) == 0)
			% because viewmtx is slightly inaccurate due to round-off in conversion from degrees to radians and then taking sines and cosines of the result
			curT = round(curT);
		end
	end
...
%	inverse transform
	...
	pts_part1 = tmp3(1:3,:)./(ones(3,1)*tmp3(4,:)) .* axr11;
	pts_part2 = axm11;
	pts = pts_part1 + pts_part2;
	pts( equalWithinOneBitRoundoff(pts_part1,-pts_part2) ) = 0;
...
function areSame = equalWithinOneBitRoundoff(x,y,bits)
% EQUALWITHINONEBITROUNDOFF  Check if numbers are equal within 1 bit roundoff
%
%   EQUALWITHINONEBITROUNDOFF(X,Y) is the same as X==Y but allows for a
%   one-bit round-off difference between X and Y.
%   
%   EQUALWITHINONEBITROUNDOFF(X,Y,BITS) does the same but ignores the last
%   BITS bits (i.e., if not provided, BITS defaults to 1).
%
% See also ==, EQ.

% Copyright (c)2021, Prof. Erik A. Johnson <JohnsonE@usc.edu>, 06/26/21

% inputs
narginchk(2,3)
if nargin<3, bits=1; end;

% easy part if they are identically equal
areSame = x == y;
if all(areSame(:)), return; end; % shortcut out

% make sure they are the same size
s = size(areSame);
sx = [size(x) ones(1,max(0,length(s)-ndims(x)))];
sy = [size(y) ones(1,max(0,length(s)-ndims(y)))];
rx=ones(size(sx)); s~=sx & sx~=0; rx(ans)=s(ans)./sx(ans); x=repmat(x,rx);
ry=ones(size(sy)); s~=sy & sy~=0; ry(ans)=s(ans)./sy(ans); y=repmat(y,ry);

% check any that are nearly equal
nbits = length(num2hex(x(1)));
hex = @(z) abs(num2hex(z)) * eye(nbits,nbits-bits);
areSame(~areSame(:)) = all( hex(x(~areSame(:))) == hex(y(~areSame(:))), 2);
