function [d, theta, Q, eq_circ] = dist_arco_rcp(Cx,Cy,R,Xarc,Yarc,Px,Py)
    % help dist_arco
    % [INPUTs]: 
    ...
    % A = [Ax, Ay];
    % B = [Bx, By];
    % result = checkPointInArc(C, R, A, B, Q);
    result = ismember(round(Q(1)),round(Xarc)) && ismember(round(Q(2)),round(Yarc));

    if result
    %     disp('Point Q is in the arc.')
    else
    %     disp('Point Q is not in the arc.')
        d = 99999;
    end
