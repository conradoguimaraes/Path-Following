...
if (singleFigure == true)
    ...
    axis(ax1,'square')
    text(h1,x0-8,y0,"$(P_{x_0}, P_{y_0})$",'Color','red','FontSize',12,'Interpreter','latex')

    titulo1 = "Vehicle Position (t="+string(t)+"s) ";
    L0string = "\{L0="+string(L0)+",";
    AlfaString = " \alpha="+string(alfa)+",";
    KvaluesString = " Kp="+string(Kp)+"\}";
    title(h1,titulo1+L0string+AlfaString+KvaluesString)
