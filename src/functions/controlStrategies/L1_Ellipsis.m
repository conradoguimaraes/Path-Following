function [cost,handler1,handler3] = L1_Ellipsis(simulationTime,Px, Py, Dx1, Dy1, Ex2, Ey2, Ex1, Ey1, Dx2, Dy2, R1, R2, Cx1, Cy1, Cx2, Cy2, state, threshold, Q, L1)
...
% 
    ...
    for t = 0:h:T
        ...
        %-----------------------------------------
        if (state == 0)
            [d, ~, ~, ~, Q] = dist_reta(Dx2, Dy2, Ex2, Ey2, x, y);
        ...
        elseif (state == 2)
            [d, lambda1, lambda2, ~, Q] = dist_reta(Ex1, Ey1, Dx1, Dy1, x, y);
...
%         dth = v*(Kp*(phid-th));
    ...
    run('figure_Ellipsis_afterFORLoop_adjustments_M3.m')
