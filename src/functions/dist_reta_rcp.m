function [d, lambda1, lambda2, theta, Q] = dist_reta_rcp(Ax,Ay,Bx,By,Px,Py)
% help dist_reta
% [INPUTs]: 
%     Coordenadas A (Ax, Ay)
%     Coordenadas B (Bx, Ay)
...
%Corrigir "bug" em que é assumido o prolongamento
%da linha (quando é horizontal ou vertical no rcp)
%e que devolve um ponto Q que não existe, sendo
%calculado o estado errado.
...
elseif (declive == Inf || declive == -Inf)
    %Reta Vertical
    if (Ay > By)
        %A em cima e B em baixo
        if (Py > Ay || Py < By)
            d = 99999;
        end
    else
        %A em baixo e B em cima
        if (Py > By || Py < Ay)
            d = 999999;
        end
    end
